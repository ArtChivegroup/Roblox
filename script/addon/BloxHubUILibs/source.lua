--[[
    ╔═══════════════════════════════════════════════════════════╗
    ║           🧩 BloxHub GUI Framework v2.0 (Fixed)           ║
    ...
    local function startListening()
        ...
        BloxHub.State.ListeningHotkey = {
            ...
            Callback = function(key)
                ...
                listening = false
                BloxHub.State.ListeningHotkey = nil
                if callback then
                    callback(key)
                end
            end,
            Cancel = function()
                -- Callback called when cancelled (e.g., Escape)
...
function BloxHub.Elements.CreateDropdown(parent, text, options, defaultIndex, callback, opts)
    ...
    Container.BackgroundColor3 = BloxHub.Settings.Theme.Secondary
    Container.BorderSizePixel = 0
    Container.ClipsDescendants = false -- Allow dropdown to overflow
    Container.ZIndex = 5
    Container.Parent = parent
    ...
    Button.AutoButtonColor = false
    Button.ClipsDescendants = true
    Button.ZIndex = 6 -- Ensure button itself is above dropdown when closed
    Button.Parent = Container
    ...
    Arrow.ZIndex = 6 -- Match button ZIndex
    Arrow.Parent = Button
    ...
    DropdownFrame.ZIndex = 10 -- Higher ZIndex to appear above other elements
    ...
    for i, option in ipairs(options) do
        ...
        OptionButton.ZIndex = 11 -- Higher than dropdown frame
...
function BloxHub.Elements.CreateIcon(parent, text, callback, options)
    options = options or {}
    local Icon = Instance.new("TextButton")
    Icon.Size = options.Size or UDim2.new(0, 30, 0, 30)
    Icon.Position = options.Position or UDim2.new(0, 0, 0, 0)
    Icon.BackgroundColor3 = options.Color or BloxHub.Settings.Theme.Secondary
    Icon.Text = text or "ℹ️"
    ...
    Icon.MouseLeave:Connect(function()
        BloxHub.Core.Tween(Icon, {BackgroundColor3 = options.Color or BloxHub.Settings.Theme.Secondary}, 0.2)
        BloxHub.Core.Tween(UIStroke, {Transparency = 0.6}, 0.2)
    end)
...
-- Perbaikan CreatePopup: Sekarang mengembalikan objek dengan metode Show
function BloxHub.Elements.CreatePopup(title, content, buttons)
    local PopupFrame = Instance.new("Frame")
    PopupFrame.Size = UDim2.new(0, 0, 0, 0)
    PopupFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
    PopupFrame.AnchorPoint = Vector2.new(0.5, 0.5)
    PopupFrame.BackgroundColor3 = BloxHub.Settings.Theme.Primary
    PopupFrame.BorderSizePixel = 0
    PopupFrame.ZIndex = 100
    -- Tidak diparent ke ScreenGui dulu
    ...
    ContentLabel.Size = UDim2.new(1, -40, 0, 60)
    ContentLabel.Position = UDim2.new(0, 20, 0, 55)
    ContentLabel.BackgroundTransparency = 1
    ContentLabel.Text = content
    ContentLabel.TextColor3 = BloxHub.Settings.Theme.TextDim
    ContentLabel.TextSize = 13
    ContentLabel.Font = BloxHub.Settings.Font.Regular
    ...
    for _, buttonData in ipairs(buttons) do
        ...
        Button.MouseButton1Click:Connect(function()
            ...
            BloxHub.Core.Tween(PopupFrame, {Size = UDim2.new(0, 0, 0, 0)}, 0.2)
    ...
    local PopupObject = {}
    function PopupObject:Show(position)
        if not BloxHub.UI.ScreenGui then return end -- Pastikan ScreenGui ada

        PopupFrame.Parent = BloxHub.UI.ScreenGui
        Shadow.Parent = BloxHub.UI.ScreenGui

        if position then
            PopupFrame.Position = position
            PopupFrame.AnchorPoint = Vector2.new(0, 0) -- Atur anchor point ke kiri atas jika posisi spesifik diberikan
        else
            PopupFrame.Position = UDim2.new(0.5, 0, 0.5, 0) -- Default ke tengah
            PopupFrame.AnchorPoint = Vector2.new(0.5, 0.5)
        end

        -- Animasi muncul
        BloxHub.Core.Tween(PopupFrame, {Size = UDim2.new(0, 400, 0, 180)}, 0.3, Enum.EasingStyle.Back)
        BloxHub.Core.Tween(Shadow, {ImageTransparency = 0.7}, 0.3)
    end
...
-- ═══════════════════════════════════════════════════════════
-- LAYOUT HELPERS (Fixed to accept parent)
-- ═══════════════════════════════════════════════════════════
...
function BloxHub:CreateWindow(title, options)
    ...
    function Window:CreateTab(tabName, options)
        ...
        function Tab:AddIcon(text, callback, options)
            local icon = BloxHub.Elements.CreateIcon(self.Content, text, callback, options)
            table.insert(self.Elements, icon)
            return icon
        end
        ...
        -- Perubahan: Tab:AddPopup sekarang mengembalikan objek popup, bukan langsung menampilkannya
...
-- ═══════════════════════════════════════════════════════════
-- INPUT HANDLING (Fixed for mobile compatibility)
-- ═══════════════════════════════════════════════════════════
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if self.State.ListeningHotkey then
            ...
            if input.KeyCode == Enum.KeyCode.Escape then
                self.State.ListeningHotkey.Cancel() -- Panggil callback cancel
                return
            end
            ...
            -- Gunakan input.KeyCode langsung untuk Enum.KeyCode
...
function BloxHub:Initialize()
    print("╔═══════════════════════════════════════════════╗")
    print("║     🧩 BloxHub GUI Framework v2.0 (Fixed)     ║")
...
function BloxHub:CreateDefaultExample()
    ...
    InfoTab:AddButton("Show Popup", function()
        -- Gunakan objek popup yang dikembalikan
        local popup = InfoTab:AddPopup("Example Popup", "This is a popup example with multiple buttons!", {
        ...
        -- Tampilkan popup di posisi default (tengah)
        popup:Show()
    end, {Icon = "💬"})
